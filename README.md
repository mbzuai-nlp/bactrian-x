...
<!---
-->
* **[2023.04.24]** Monolingual models in 52 languages are released, and are available at [Hugging Face Model Hub](https://huggingface.co/haonan-li).
* **[2023.04.22]** Data in 52 languages are released, and are available at [Hugging Face Datasets Hub](https://huggingface.co/datasets/MBZUAI/Bactrian-X).
...
- The [67K instruction data](#data-release) in 52 languages.
- Multilingual [Bactrian-X](#data-release), trained on combined language-instruction pairs (3.4M instances).
...
Replace `lang` with a language code, e.g., `en` for `English`; or a list of `lang` code separated by comma `,`, e.g., `en,zh` for `English` and `Chinese`.


### Inference

This file reads the foundation model from the Hugging Face model hub and the LoRA weights from `local`, and runs a Gradio interface for inference on a specified input. Users should treat this as example code for the use of the model, and modify it as needed.
```bash
python generate.py \
    --load_8bit \
    --base_model 'decapoda-research/llama-7b-hf' \
