...
class DataArguments:
    ...
    val_set_size: Optional[int] = field(default=2000, metadata={"help": ""})
...
class BactrianTrainingArguments(TrainingArguments):
    ...
    train_on_inputs: Optional[bool] = field(default=False, metadata={"help": ""})
    lang: str = field(default="zh", metadata={"help": "The languages."})
    template_dir: str = field(default="./templates", metadata={"help": ""})
    prompt_template_name: str = field(default="bactrian", metadata={"help": ""})
    evaluation_strategy: str = field(default="steps", metadata={"help": ""})
    save_strategy: str = field(default="steps", metadata={"help": ""})
    wandb_project: str = field(default="bactrian", metadata={"help": ""})
...
def train():
    ...
    config = LoraConfig(
        ...
        bias = "none",
        task_type = "CAUSAL_LM",
        # modules_to_save = ["lm_head"] if args.tune_embedding else None , # embed_tokens
    )
    ...
    def get_example_length(example):
        ...
        example["example_length"] = len(tokenizer(full_prompt + tokenizer.eos_token)["input_ids"])
        return example

    if model_max_length is None: # (Time consuming) Not recommend.
